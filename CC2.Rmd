...
### Votos a favor {.value-box}

```{r box-votos-favor}
valueBox(212, color = "#b2df8a") 
```

### Votos en contra {.value-box}

```{r box-votos-contra}
valueBox(36, color = "#fb9a99") 
```

### Abstención {.value-box}
...
```{r viz-region, width = 20, height = 5, fig.align='right'}
  ggplot(suma_region, aes(y = reorder(region, sumaregion), x = sumaregion, 
                          fill = ToHighlight )) + 
  geom_bar(stat = "identity", position = "dodge") + 
  theme_classic() +
  labs(title ="", x = "Número participantes", y = "") +
  geom_text(aes(label = sumaregion), color = "black", size=3, 
            face="bold", vjust = 0.5, hjust = -0.2) +
...
```{r box-total-com}
valueBox(nrow(comments), icon = "fa-comments") 
```

Column 
-----------------------------------------------------------------------

### Comentarios con número de "me gusta"

```{r me-gusta}
comments %>%
  select(comment, me_gusta, no_me_gusta) %>%
  arrange(desc("me_gusta")) %>%
  kable(col.names = c('Comentario', '"Me gusta"',
                      '"No me gusta"'),
               align = c('lcc')) %>%
